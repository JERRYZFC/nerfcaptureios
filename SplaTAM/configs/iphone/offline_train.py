...
# num_frames = 10 # No longer needed, determined from len(frames)

downscale_factor = 2.0
densify_downscale_factor = 4.0

map_every = 1
keyframe_every = 5 # Or adjust as needed
mapping_window_size = 32
tracking_iters = 60
mapping_iters = 60

# Determine full resolution from manifest later in the training script if needed
# For now, we can set placeholder values or calculate them if manifest is pre-read.
# This config assumes the training script will handle resolution dynamically.
full_res_width = 1920
full_res_height = 1440


config = dict(
    workdir=f"./experiments/offline/{os.path.basename(base_dir)}",
    ...
    viz=dict(
        ...
        enter_interactive_post_online=True, # Enter Interactive Mode after Offline Recon Viz
    ),
)
